...
def buildExercise5Rubric() -> GradingRubric:
    ...
    rubric.addCheck(
        ...
        ensureColumns,
        2,
        "Ensure all 5 columns of the source data are imported.",
    )
    rubric.addCheck(
        "Shape",
        ensureRows,
        2,
        "Ensure all 62 complete data points of the source data are imported.",
    )
    rubric.addCheck(
        "Types", ensureTypes, 5, "Ensure all columns have a fitting basic data type."
    )
    rubric.addCheck(
        "Quality",
        ensureNoEmptyValues,
    ...
    rubric.addCheck(
        ...
        ensureZoneId,
        2,
        "Ensure only stops from the right zone are included in the dataset.",
    )
    rubric.addCheck(
        "Quality",
        ensureEncoding,
